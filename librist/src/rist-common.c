...
static void send_nack_group(struct rist_receiver *ctx, struct rist_flow *f)
{
	...
	// First pass: Look for weight-1000 recovery agents and select the one with lowest RTT
	...
	{
		// No recovery agent found, use standard logic (lowest RTT)
		for (size_t i = 0; i < f->peer_lst_len; i++)
		{
			struct rist_peer *check = f->peer_lst[i];
			if (check->is_rtcp && !check->dead && check->last_rtt < last_rtt)
...
#ifdef defaut_rist_recv_rtcp_function
		...
		switch(ptype) {
			case PTYPE_NACK_CUSTOM:
				if (subtype == NACK_FMT_SEQEXT)
				{
					...
					// Minimum size: rtcp_hdr (8) + name (4) = 12 bytes
					if (bytes_left < 12) {
						rist_log_priv(ctx, RIST_LOG_ERROR, "RTCP FSR packet too small: %zu bytes\n", bytes_left);
						break;
					}
					
					// Parse name field manually - it's at offset 8 after rist_rtcp_hdr
					uint8_t *name = pkt + 8;
					
					/* Validate this is a RIST packet */
					if (memcmp(name, "RIST", 4) != 0) {
						rist_log_priv(ctx, RIST_LOG_DEBUG, "Non-RIST APP packet (name: %.4s), ignoring\n", name);
						break;
					}
					
					/* Only process FSR requests on sender side */
					if (peer->receiver_mode) {
						rist_log_priv(ctx, RIST_LOG_DEBUG, "Received FSR request on receiver, ignoring\n");
						break;
					}
					
					/* Validate peer is authenticated */
					if (!peer->authenticated) {
						rist_log_priv(ctx, RIST_LOG_WARN, "FSR request from unauthenticated peer, ignoring\n");
						break;
					}
					...
					if (subtype == FSR_SUBTYPE_ENABLE) {
						if (add_to_fsr_list(peer->adv_peer_id) == 0) {
							rist_log_priv(ctx, RIST_LOG_INFO, 
									"FSR ENABLE: Added peer %"PRIu32" to full stream list\n",
									peer->adv_peer_id);
						} else {
							rist_log_priv(ctx, RIST_LOG_WARN, 
									"FSR ENABLE: Failed to add peer %"PRIu32"\n",
									peer->adv_peer_id);
						}
					}
						...
						if (remove_from_fsr_list(peer->adv_peer_id) == 0) {
							rist_log_priv(ctx, RIST_LOG_INFO, 
									"FSR DISABLE: Removed peer %"PRIu32" from full stream list\n",
									peer->adv_peer_id);
						} else {
							rist_log_priv(ctx, RIST_LOG_DEBUG, 
									"FSR DISABLE: Peer %"PRIu32" was not in list\n",
									peer->adv_peer_id);
						}
					}
					break;
...
#ifdef default_rist_timeout_check_function
...
/* =============================================================================
 * Complete rist_timeout_check with FSR cleanup
 * =============================================================================
 */
	...
	while (peer)
	{
		...
		} else if (peer->dead && peer->parent)
		{
			if ( peer->dead_since < now && (now - peer->dead_since) > 5000 * RIST_CLOCK)
			{
				...
				if (!peer->receiver_mode) {
					// Only clean up FSR for sender-side peers
					int result = remove_from_fsr_list(peer->adv_peer_id);
					if (result == 0) {
						rist_log_priv2(cctx->logging_settings, RIST_LOG_INFO, 
							"FSR CLEANUP: Removed timed-out peer %u from FSR list\n", peer->adv_peer_id);
					}
					// If result == -1 (not found), no log needed - peer wasn't using FSR
				}
...
/* =============================================================================
 * Complete rist_peer_remove with FSR cleanup
 * =============================================================================
 */
...
#ifdef default_rist_peer_remove_function
...
if (peer && peer->adv_peer_id) {
	...
	if (!peer->receiver_mode) {
		...
		int result = remove_from_fsr_list(peer->adv_peer_id);
		if (result == 0) {
			rist_log_priv2(ctx->logging_settings, RIST_LOG_DEBUG, 
				"FSR CLEANUP: Removed peer %u from FSR list during peer removal\n", peer->adv_peer_id);
		}
		// If result == -1 (not found), no log needed - peer wasn't using FSR
	}
...
/*
RIST_TIMEOUT_CHECK CHANGES:
- Added FSR cleanup in the "dead && peer->parent" branch
- Only for sender-side peers (!peer->receiver_mode)
- Logs when FSR peer actually removed (result == 0)
- Silent when peer not in FSR list (result == -1)

RIST_PEER_REMOVE CHANGES:
- Added FSR cleanup at the very beginning (after NULL check)
- Ensures FSR cleanup happens for ALL peer removals
- Same logic: sender-only, log when removed, silent when not found

DOUBLE COVERAGE:
- rist_timeout_check() catches timeout-based removals
- rist_peer_remove() catches ALL other removals
- No FSR entries can be orphaned

THREAD SAFETY:
- Your FSR functions already handle pthread locking
- No additional locking needed in these functions
*/
...
#ifdef ORIGINAL_RECEIVER_FUNCTION
...
PTHREAD_START_FUNC(receiver_pthread_protocol, arg)
{
	...
	while (!atomic_load_explicit(&ctx->common.shutdown, memory_order_acquire)) {
		...
		pthread_mutex_lock(&ctx->common.peerlist_lock);
		bool has_peers = (ctx->common.PEERS != NULL);
		pthread_mutex_unlock(&ctx->common.peerlist_lock);
		...
		// FSR (Full Stream Request) logic with 50% loss and 2-second timeout detection
		{
			// Initialize FSR system once we have peers
			if (!fsr_initialized) {
				fsr_initialized = true;
				fsr_check_next_time = now;
				last_debug_time = now;
				rist_log_priv(&ctx->common, RIST_LOG_INFO, "FSR: System initialized\n");
			}
			
			// Check FSR conditions every second
			if (now >= fsr_check_next_time) {
				fsr_check_next_time = now + ONE_SECOND;
				
				pthread_mutex_lock(&ctx->common.peerlist_lock);
				struct rist_peer *peer = ctx->common.PEERS;
				bool should_enable_fsr = false;
				...
				bool debug_this_cycle = (now >= (last_debug_time + (10 * ONE_SECOND)));
				if (debug_this_cycle) {
					last_debug_time = now;
					rist_log_priv(&ctx->common, RIST_LOG_INFO, "FSR: Debug - FSR active: %s\n", 
						fsr_active ? "YES" : "NO");
				}
				...
				while (peer) {
					if (peer->is_data && peer->flow) {
						if (debug_this_cycle) {
							rist_log_priv(&ctx->common, RIST_LOG_INFO, 
								"FSR: Debug - Peer ID %u, weight %u, is_data %d\n", 
								peer->adv_peer_id, (unsigned int)peer->config.weight, peer->is_data);
						}
						
						// Check if this is the satellite peer (not weight 1000)
						if (peer->config.weight != 1000) {
							sat_peer_count++;
							sat_peer = peer;  // Store reference to the single satellite peer
						}
						// Check if this is a recovery peer (weight 1000)
						else if (peer->config.weight == 1000) {
							...
							if (debug_this_cycle) {
								rist_log_priv(&ctx->common, RIST_LOG_INFO, 
									"FSR: Debug - Recovery peer %u: received=%llu\n", 
									peer->adv_peer_id, (unsigned long long)peer->stats_receiver_instant.received);
							}
						}
					}
					peer = peer->next;
				...
				// Analyze the single satellite peer for FSR decisions
				if (sat_peer) {
					// Monitor ONLY satellite peer statistics - not flow statistics
					uint64_t sat_received = sat_peer->stats_receiver_instant.received;
					...
					if (sat_peer_dead) {
						...
						rist_log_priv(&ctx->common, RIST_LOG_WARN,
							"FSR: Satellite peer %u is DEAD\n", sat_peer->adv_peer_id);
					}
					// Condition 2: No packets from satellite peer for 2+ seconds
					else if (time_since_sat_packet > 2000) {
						should_enable_fsr = true;
						rist_log_priv(&ctx->common, RIST_LOG_WARN,
							"FSR: Satellite peer %u timeout - no packets for %llu ms\n", 
							sat_peer->adv_peer_id, (unsigned long long)time_since_sat_packet);
					}
					// Condition 3: High RTT indicates link degradation
					...
					if (fsr_active && !should_enable_fsr) {
						...
						if (!sat_peer_dead && 
							...
							if (debug_this_cycle || fsr_active) {
								rist_log_priv(&ctx->common, RIST_LOG_INFO,
									"FSR: Satellite peer %u healthy - disabling FSR\n", sat_peer->adv_peer_id);
							}
						}
					}
				}
				...
				if (should_enable_fsr && !fsr_active) {
					...
					if (recovery_agent) {
						...
						if (sat_peer && sat_peer->flow) {
							// Normal case: use satellite peer's flow ID
							flow_id = sat_peer->flow->flow_id;
							rist_log_priv(&ctx->common, RIST_LOG_INFO,
								"FSR: Sending FSR Enable via recovery agent %u for satellite flow %u\n", 
								recovery_agent->adv_peer_id, flow_id);
						} else {
							// No satellite peer case: use recovery peer's flow ID
							flow_id = recovery_agent->flow ? recovery_agent->flow->flow_id : 0;
							rist_log_priv(&ctx->common, RIST_LOG_INFO,
								"FSR: Sending FSR Enable via recovery agent %u (no satellite peer, using flow %u)\n", 
								recovery_agent->adv_peer_id, flow_id);
						}
						...
						if (flow_id > 0 && rist_send_fsr_enable(recovery_agent, flow_id) >= 0) {
							...
							rist_log_priv(&ctx->common, RIST_LOG_ERROR,
								"FSR: Failed to send FSR Enable - invalid flow ID (%u)\n", flow_id);
						}
					} else {
						rist_log_priv(&ctx->common, RIST_LOG_ERROR,
							"FSR: Cannot send FSR Enable - no recovery agent available\n");
					}
					...
					if ((now - last_fsr_enable_time) >= (30 * ONE_SECOND)) {
						...
						if (recovery_agent) {
							if (rist_send_fsr_enable(recovery_agent, sat_peer->flow->flow_id) >= 0) {
								last_fsr_enable_time = now;
								rist_log_priv(&ctx->common, RIST_LOG_INFO,
									"FSR: Sent keepalive FSR Enable (30s timer)\n");
							}
						}
					}
				}
		...
		// Limit scope of `struct rist_flow *f` for clarity since it is used again later in this loop.
		{
			...
			while (f) {
				struct rist_flow *next = f->next;
				pthread_mutex_lock(&f->mutex);
				if (f->dead && (now - f->last_recv_ts) > f->flow_timeout) {
					rist_log_priv(&ctx->common, RIST_LOG_INFO,
							"Deleting dead flow with peer_lst_len = %zu\n", f->peer_lst_len);
					if (f->peer_lst_len) {
						for (size_t i = 0; i < f->peer_lst_len; i++) {
							struct rist_peer *peer = f->peer_lst[i];
							peer->flow = NULL;
