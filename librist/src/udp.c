...
// Dynamic array of peer IDs that have requested full stream recovery
...
{
	pthread_mutex_lock(&fsr_lock);
	for (int i = 0; i < fsr_peer_count; i++) {
		if (fsr_peers[i] == peer_id) {
			pthread_mutex_unlock(&fsr_lock);
			return true;
		}
	}
	pthread_mutex_unlock(&fsr_lock);
...
RIST_PRIV int add_to_fsr_list(uint32_t peer_id)
{
	...
	if (fsr_peer_count >= fsr_peers_capacity) {
		int new_capacity = fsr_peers_capacity == 0 ? 8 : fsr_peers_capacity * 2;
		uint32_t *new_array = realloc(fsr_peers, new_capacity * sizeof(uint32_t));
		if (!new_array) {
			pthread_mutex_unlock(&fsr_lock);
			return -1; // Out of memory
		}
		fsr_peers = new_array;
		fsr_peers_capacity = new_capacity;
	}
	
	fsr_peers[fsr_peer_count] = peer_id;
	fsr_peer_count++;
	pthread_mutex_unlock(&fsr_lock);
	return 0;
}

// Remove peer ID from FSR list  
RIST_PRIV int remove_from_fsr_list(uint32_t peer_id)
{
	...
	for (int i = 0; i < fsr_peer_count; i++) {
		if (fsr_peers[i] == peer_id) {
			...
			for (int j = i; j < fsr_peer_count - 1; j++) {
				fsr_peers[j] = fsr_peers[j + 1];
			}
			fsr_peer_count--;
			
			// Shrink array if it's using less than 25% of capacity and capacity > 8
			if (fsr_peers_capacity > 8 && fsr_peer_count < fsr_peers_capacity / 4) {
				int new_capacity = fsr_peers_capacity / 2;
				uint32_t *new_array = realloc(fsr_peers, new_capacity * sizeof(uint32_t));
				if (new_array) { // Only shrink if realloc succeeds
					fsr_peers = new_array;
					fsr_peers_capacity = new_capacity;
				}
			}
			
			// Free array completely if empty
			if (fsr_peer_count == 0) {
				free(fsr_peers);
				fsr_peers = NULL;
				fsr_peers_capacity = 0;
			}
...
// Check if any peers have requested FSR
RIST_PRIV bool has_fsr_requests(void)
{
	pthread_mutex_lock(&fsr_lock);
	bool has_requests = (fsr_peer_count > 0);
	pthread_mutex_unlock(&fsr_lock);
	return has_requests;
}
...
peer_select:
	...
	for (peer = ctx->common.PEERS; peer; peer = peer->next) {
		...
		if (peer->config.weight == 0 && !looped) {
			// Standard zero-weight peers always send
			if (peer->listening) {
			...
			if (has_fsr_requests()) {
				if (peer->listening) {
...
#endif
						// Check if this child's peer_id is in the FSR list
						if (child->authenticated && child->is_data && peer_id_in_fsr_list(child->adv_peer_id)
							&& (!child->dead || (child->dead && (child->dead_since + peer->recovery_buffer_ticks) < now))) {
							uint8_t *payload = buffer->data;
							rist_send_common_rtcp(child, buffer->type, &payload[RIST_MAX_PAYLOAD_OFFSET], buffer->size, buffer->source_time, buffer->src_port, buffer->dst_port, buffer->seq_rtp);
						}
						child = child->sibling_next;
					...
					// For outbound connections, send if this peer's ID is in FSR list
...
#endif
					if (child->authenticated && child->is_data && (!child->dead || (child->dead && (child->dead_since + peer->recovery_buffer_ticks) < now))) {
						send_filtered_data_to_peer(child, buffer, buffer->type, buffer->source_time, buffer->src_port, buffer->dst_port, buffer->seq_rtp);
					}
					child = child->sibling_next;
				}
			} else if (!peer->dead || (peer->dead && (peer->dead_since + peer->recovery_buffer_ticks) < now)) {
...
ssize_t rist_retry_dequeue(struct rist_sender *ctx)
{
	...
	if (ctx->require_selection &&
		...
		refused_since++;
		if (now_refuse > last_refuse_log + (uint64_t)ONE_SECOND) {
...
int rist_send_fsr_enable(struct rist_peer *peer, uint32_t flow_id) {
    uint8_t *rtcp_buf = get_cctx(peer)->buf.rtcp;
    int payload_len = 0;
    
    rist_rtcp_write_fsr_enable(rtcp_buf, &payload_len, flow_id);
    
    // Follow the same pattern as other RTCP functions in udp.c
    // Pass peer->remote_port - the function will use virt_dst_port if remote_port is 0
    int ret = rist_send_common_rtcp(peer, RIST_PAYLOAD_TYPE_RTCP, 
    ...
    if (ret >= 0) {
        ...
        if (add_to_fsr_list(peer->adv_peer_id) == 0) {
            rist_log_priv(get_cctx(peer), RIST_LOG_INFO,
                "FSR: Added peer %u to active FSR list\n", peer->adv_peer_id);
        }
    }
...
int rist_send_fsr_disable(struct rist_peer *peer, uint32_t flow_id) {
    uint8_t *rtcp_buf = get_cctx(peer)->buf.rtcp;
    int payload_len = 0;
    
    rist_rtcp_write_fsr_disable(rtcp_buf, &payload_len, flow_id);
    ...
    if (ret >= 0) {
        rist_log_priv(get_cctx(peer), RIST_LOG_INFO, 
            "FSR: Sent FSR Disable for flow %u\n", flow_id);
            
        // Remove peer from FSR list
        if (remove_from_fsr_list(peer->adv_peer_id) == 0) {
            rist_log_priv(get_cctx(peer), RIST_LOG_INFO,
                "FSR: Removed peer %u from active FSR list\n", peer->adv_peer_id);
        }
    }
